...
def _load_signal_groups(tokens: DbcTokens, attributes: DbcAttributes) -> defaultdict[int, list[SignalGroup]]:
    ...
    def get_signal_attributes(frame_id_dbc: int, signal_short_name: str) -> DbcAttributeMap:
        ...
        frame_signal_attribs: dict[str, DbcAttributeMap]|None = attributes.signals.get(frame_id_dbc)
        if frame_signal_attribs is None:
            return OrderedDict()

        if (signal_attribs := frame_signal_attribs.get(signal_short_name)) is not None:
