...
def _load_signal_groups(tokens: DbcTokens, attributes: DbcAttributes) -> defaultdict[int, list[SignalGroup]]:
    ...
    def get_signal_attributes(frame_id_dbc: int, signal_short_name: str) -> DbcAttributeMap:
        ...
        frame_signal_attribs: dict[str, DbcAttributeMap] = \
            attributes.signals.get(frame_id_dbc, typing.cast("dict[str, DbcAttributeMap]", EMPTY_DICT))
...
def _load_signals(tokens: list[MatchObject],
    ...
    frame_mux_values: MuxSignalValues = \
        signal_multiplexer_values.get(frame_id_dbc, typing.cast("MuxSignalValues", EMPTY_DICT))
    ...
    for _message in tokens.get('BO_', []):
        ...
        messages.append(
            Message(frame_id=frame_id,
                    ...
                    name=get_message_long_name(frame_id_dbc, dbc_assert_type(message[2], str)),
